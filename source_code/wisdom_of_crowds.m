...
toc
 ...
 [B,I]=sort(Total_Distance,'ascend');
 population_woc = New_Pop(I(1,1:Population_Size),:);
...
matrix=[population_woc;path];

d = distances(path(1,n),path(1,1)); 
            for k = 2:n
                d = d + distances(path(1,k-1),path(1,k));
            end
cost=[B';d];
max_agreement=size(population_woc,1);

[x,y]=plot_agreement(matrix,cost,max_agreement);
figure(10)
plot(x(1,1:end-1),y(1,1:end-1),'*')
hold on
plot(x(1,end),y(1,end),'ro','MarkerFaceColor','r')
